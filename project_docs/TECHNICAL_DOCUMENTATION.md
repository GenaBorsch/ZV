...
- **Загрузка файлов**: MinIO интеграция для аватаров персонажей и пользователей
- **Story Pool**: База игровых элементов (монстры, текстовые события) с блокировкой
...
#### Игровой процесс
- **`Character (characters)`**: playerId (→ PlayerProfile), name, **avatarUrl?** (MinIO URL), archetype?, level, **sheetUrl?** (MinIO URL), backstory?, journal?, isAlive, deathDate?, notes?
...
- **`Report (reports)`**: groupId, masterId, description, highlights?, status (PENDING|APPROVED|REJECTED|CANCELLED), rejectionReason?, createdAt, updatedAt
- **`ReportPlayer (report_players)`**: reportId (→ Report), playerId (→ User), createdAt - связь отчётов с игроками
- **`ReportNextPlan (report_next_plans)`**: reportId, continuedFromReportId?, nextPlanText, monsterId, locationTextId, mainEventTextId, sideEventTextId - план следующей игры
...
- **`Monster (monsters)`**: title, imageUrl?, description, lastKnownLocation?, bountyAlive?, bountyDead?, status (AVAILABLE|LOCKED), lockedByReportId?, lockedByGroupId?, lockedAt?, isActive
- **`StoryText (story_texts)`**: type (LOCATION|MAIN_EVENT|SIDE_EVENT), text, status (AVAILABLE|LOCKED), lockedByReportId?, lockedByGroupId?, lockedAt?, isActive
...
- **`GET/POST /api/reports`** - список отчётов (роль-зависимый) и создание отчётов (только мастера)
  ...
  - Поддержка Story Pool: выбор монстра и текстовых элементов для следующей игры
...
- **`PATCH /api/reports/[id]`** - редактирование отчёта (мастер) или модерация (админ)
  ...
  - ✅ При одобрении автоматически списываются игры с баттлпассов и блокируются элементы Story Pool
...
#### Story Pool (База игровых элементов):
- **`GET /api/admin/monsters`** - список монстров для админ-панели
- **`POST /api/admin/monsters`** - создание монстра
- **`PATCH /api/admin/monsters/[id]`** - редактирование монстра
- **`POST /api/admin/monsters/[id]/unlock`** - разблокировка монстра
- **`GET /api/admin/story-texts`** - список текстовых элементов для админ-панели
- **`POST /api/admin/story-texts`** - создание текстового элемента
- **`POST /api/admin/story-texts/[id]/unlock`** - разблокировка текстового элемента
- **`GET /api/story-pool/monsters/available`** - доступные монстры для мастеров
- **`GET /api/story-pool/texts/available`** - доступные текстовые элементы
- **`POST /api/story-pool/feeling-lucky`** - случайная генерация сетки событий
- **`GET /api/story-pool/continuation`** - получение плана продолжения для группы
...
**Последнее обновление**: Декабрь 2024 - добавлена безопасная система управления файлами с MinIO
...
**Story Pool** — система выбора игровых элементов для следующей игры при создании отчёта мастером.

#### Основные сущности:

**Монстры (`monsters`):**
- Карточки монстров с изображениями
- Поля: название, описание, последняя известная локация, награды
- Блокировка при использовании в одобренном отчёте

**Текстовые элементы (`story_texts`):**
- Три типа: LOCATION, MAIN_EVENT, SIDE_EVENT
- Блокировка при использовании
- Один элемент не может использоваться дважды

#### Процесс работы:

1. **Мастер создаёт отчёт** с планом следующей игры:
   - Выбирает монстра из доступных
   - Выбирает 3 текстовых элемента (локация, основное событие, второстепенное)
   - Пишет текстовый план (до 2000 символов)
   - Или использует кнопку "Мне повезёт" для случайной генерации

2. **При одобрении отчёта** (APPROVED):
   - Элементы автоматически блокируются
   - Привязываются к отчёту и группе
   - Становятся недоступными другим мастерам

3. **Админ может разблокировать** элементы вручную при необходимости

#### API эндпоинты:

- `/api/admin/monsters` — управление монстрами (CRUD)
- `/api/admin/story-texts` — управление текстовыми элементами (CRUD)
- `/api/story-pool/monsters/available` — доступные монстры для выбора
- `/api/story-pool/texts/available` — доступные тексты по типу
- `/api/story-pool/feeling-lucky` — случайная генерация сетки
- `/api/story-pool/continuation` — получение плана продолжения для группы

#### Безопасность:

- Оптимистическая блокировка — проверка доступности при создании отчёта
- Атомарные транзакции при блокировке элементов
- Обработка конфликтов при одновременном выборе элементов
